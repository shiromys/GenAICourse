...
const AdminDashboard = () => {
    ...
    return (
        <AuroraBackground beige className="min-h-screen selection:bg-blue-500/30 pt-28 pb-12">
            <div className="container mx-auto px-6 py-8 relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                    <div>
                        <h1 className="text-4xl font-black text-[var(--text-main)] tracking-tight flex items-center gap-3">
                            <span className="text-blue-600">⚡</span> Admin console
                        </h1>
                                    ...
                                    <div className="space-y-4">
                                        {stats.popularCourses && stats.popularCourses.length > 0 ? (
                                            stats.popularCourses.map((course, idx) => (
                                                <div key={course._id || idx} className="group flex items-center justify-between p-5 bg-slate-50/50 rounded-2xl border border-slate-100 hover:border-blue-200 hover:bg-white transition-all duration-300">
                                                    <div className="flex items-center gap-5">
                                                        <div className="w-12 h-12 rounded-xl bg-white text-blue-600 flex items-center justify-center font-black text-xl border border-slate-100 shadow-sm">
                                                            {idx + 1}
                                                        </div>
                                                        <div>
                                                            <h4 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors text-lg">{course.title || 'Untitled'}</h4>
                                                    ...
                                                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
                                                        ...
                                                        <span className="font-black text-blue-600">{course.enrollmentCount || 0}</span>
                                                    </div>
                                                </div>
                                            ))
                                        ) : (
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white rounded-3xl p-8 border border-slate-200/60 shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            <button onClick={() => navigate('/admin/courses/new')} className="w-full py-4 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-100">
                                            ...
                                            {paymentAnalytics.dailyRevenue.map((day, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className="flex flex-col items-center justify-end flex-shrink-0 group w-12 h-full">
                                                        <div
                                                            className="w-full bg-gradient-to-t from-blue-600/50 to-blue-400 rounded-t-sm opacity-80 group-hover:opacity-100 transition-all relative cursor-pointer"
                                                ...
                                                paymentAnalytics.revenueByCourse.map((rc, idx) => (
                                                    <div key={idx} className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                                        <div className="flex-1 min-w-0 pr-4">
                                                            ...
                                                            <div className="text-xs text-blue-600 font-mono mt-1">{rc.sales} transactions</div>
                                                        </div>
                                    ...
                                    <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 border border-[var(--card-border)] shadow-sm">
                                        ...
                                        <div className="space-y-5">
                                            ...
                                            <div>
                                                ...
                                                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                                    <div className="h-full bg-blue-500 rounded-full" style={{ width: paymentAnalytics?.conversionMetrics?.checkoutConversionRate || '0%' }}></div>
                                                </div>
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm gap-4">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        ...
                                        className="btn bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-blue-100"
                                    >
                                        <FaPlus /> New Course
                                    </button>
                                </div>
                                            ...
                                            {courses && courses.length > 0 ? (
                                                courses.map(course => (
                                                    <tr key={course._id || course.id} className="hover:bg-slate-50/50 transition-colors group">
                                                        <td className="px-8 py-6 font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors uppercase tracking-tight">{course.title || 'Untitled'}</td>
                                    ...
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50/80 text-slate-500 uppercase text-[10px] font-black tracking-[0.2em]">
                                            <tr>
                                                ...
                                                <th className="px-8 py-5 text-blue-600 font-black italic underline decoration-blue-500/30">Sign up date</th>
                                                <th className="px-8 py-5 text-right">Directives</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100">
                                                ...
                                                users.map(u => (
                                                    <tr key={u._id || u.id} className="hover:bg-slate-50/50 transition-colors group">
                                                        <td className="px-8 py-6 font-bold text-slate-900 flex items-center gap-4">
                                                            <div className="w-10 h-10 rounded-2xl bg-slate-100 flex items-center justify-center text-sm font-black text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
...
const GlassStatCard = ({ title, value, icon }) => {
    return (
        <div className="relative overflow-hidden bg-white/80 backdrop-blur-xl rounded-3xl p-8 border border-[var(--card-border)] shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-500/50 group">
            ...
            <div className="relative z-10 flex items-center justify-between">
                ...
                <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 text-2xl shadow-lg border border-blue-100 group-hover:scale-110 transition-transform">
                    {icon}
                </div>
            </div>
        </div>
...
const StatCard = ({ title, value, icon, gradient }) => {
    return (
        <div className="relative group overflow-hidden bg-white rounded-3xl p-8 border border-slate-200/60 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-blue-100 text-slate-900">
...
const AdminActionButton = ({ onClick, icon, color, title }) => {
    const colorClasses = {
        ...
        indigo: 'text-blue-500 hover:bg-blue-50 hover:border-blue-200',
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 border border-[var(--card-border)] shadow-sm mt-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        {availableYears.map(year => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all border ${selectedYear === year
                                    ? 'bg-blue-500 text-white border-blue-400 shadow-[0_0_10px_rgba(37, 99, 235,0.3)]'
                ...
                <div className="text-left md:text-right">
                    ...
                    <p className="text-3xl font-black text-blue-600">${displayRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-2 mt-2">
                <button
                    ...
                    className={`w-full lg:w-32 py-3 overflow-hidden rounded-xl font-bold transition-all border shrink-0 ${selectedMonth === null
                        ? 'bg-blue-500 text-white border-blue-400 shadow-[0_0_15px_rgba(37, 99, 235,0.3)]'
                        : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50 hover:text-blue-600'
                        }`}
                >
                    {selectedYear}
                    ...
                    {months.map((month, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {hasRevenue && (
                                    <div className={`absolute top-1 right-1 w-1.5 h-1.5 rounded-full ${selectedMonth === idx ? 'bg-black' : 'bg-blue-500'}`}></div>
                                )}
                            </button>
                        );
                    })}
